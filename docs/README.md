# OpenClaw Trusted Mode Docs Index

This index organizes release-facing documentation for operators, implementers, and auditors.

**Product:** [dexgate.ai](https://dexgate.ai) · **Hosted quickstart:** [docs/openclaw/quickstart](https://dexgate.ai/docs/openclaw/quickstart/) · **GitHub:** [dexgate-ai/openclaw-trusted-mode](https://github.com/dexgate-ai/openclaw-trusted-mode) · **X:** [@dexgateAI](https://x.com/dexgateAI)

## Start Here

- First-time setup: [`../START_HERE.md`](../START_HERE.md)
- Troubleshooting: [`../SELF_SERVICE_FAQ.md`](../SELF_SERVICE_FAQ.md)
- Technical operations guide: [`../OPERATIONS_GUIDE.md`](../OPERATIONS_GUIDE.md)
- Non-technical runbook: [`../RUNBOOK_NON_TECHNICAL.md`](../RUNBOOK_NON_TECHNICAL.md)

## Governance and Security

- Security posture and reporting: [`../SECURITY.md`](../SECURITY.md)
- Security evidence bundle index: [`../SECURITY_EVIDENCE_BUNDLE.md`](../SECURITY_EVIDENCE_BUNDLE.md)
- Threat model summary: [`../THREAT_MODEL_SUMMARY.md`](../THREAT_MODEL_SUMMARY.md)
- Third-party notices: [`../THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md)

## Release and Readiness

- Production readiness checklist: [`../PRODUCTION_READINESS_CHECKLIST.md`](../PRODUCTION_READINESS_CHECKLIST.md)
- Public launch checklist: [`../PUBLIC_RELEASE_READINESS_CHECKLIST.md`](../PUBLIC_RELEASE_READINESS_CHECKLIST.md)
- Public release process: [`../PUBLIC_RELEASE_PROCESS_RUNBOOK.md`](../PUBLIC_RELEASE_PROCESS_RUNBOOK.md)
- Release operations: [`../RELEASE_OPERATIONS.md`](../RELEASE_OPERATIONS.md)
- Release declaration: [`../RELEASE_v1.0.0.md`](../RELEASE_v1.0.0.md)
- Versioned evidence pack: [`../release-evidence/v1.0.0/EVIDENCE_INDEX.md`](../release-evidence/v1.0.0/EVIDENCE_INDEX.md)

## Product and Compatibility

- OpenClaw compatibility certification: [`../COMPATIBILITY_MATRIX.md`](../COMPATIBILITY_MATRIX.md)
- Performance baseline: [`../PERFORMANCE_BASELINE.md`](../PERFORMANCE_BASELINE.md)
- Technical certification test pack: [`../SDE_PDP_OpenClaw_Technical_Certification_Test_Pack_v1.md`](../SDE_PDP_OpenClaw_Technical_Certification_Test_Pack_v1.md)
- Enterprise validation test pack: [`../SDE_PDP_OpenClaw_Enterprise_Validation_Test_Pack_v1.md`](../SDE_PDP_OpenClaw_Enterprise_Validation_Test_Pack_v1.md)

## Reference

- Changelog: [`../CHANGELOG.md`](../CHANGELOG.md)
- Glossary and acronyms: [`../GLOSSARY.md`](../GLOSSARY.md)
- Release docs inclusion rules: [`./release_docs_manifest.md`](./release_docs_manifest.md)
